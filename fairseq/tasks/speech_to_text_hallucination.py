...
@register_task("speech_to_text_hallucination")
class SpeechToTextHallucinationTask(LegacyFairseqTask):
    @classmethod
    def add_args(cls, parser):
        parser.add_argument("data", help="manifest root path")
        parser.add_argument("hallucination", help="hallucination root path")
        parser.add_argument(
            "--config-yaml",
            type=str,
            default="config.yaml",
    ...
    def __init__(self, args, tgt_dict):
        super().__init__(args)
        self.hallucination_datasets = dict()
    ...
    def make_batches(self,dataset, epoch=1,max_sentences=100,required_batch_size_multiple=False,):
        ...
        batches = dataset.batch_by_size(
            indices,
            max_tokens=self.args.max_source_positions,
    ...
    def load_hallucination_dataset(self, split, epoch=1, combine=False, **kwargs):
        is_train_split = split.startswith("train")
        pre_tokenizer = self.build_tokenizer(self.args)
        bpe_tokenizer = self.build_bpe(self.args)
        self.hallucination_datasets[split] = SpeechToTextDatasetCreator.from_tsv(
            root=self.args.hallucination,
            cfg=self.data_cfg,
            splits=split,
            tgt_dict=self.tgt_dict,
            pre_tokenizer=pre_tokenizer,
    ...
    def build_model(self, args, from_checkpoint=False):
        ...
        return super(SpeechToTextHallucinationTask, self).build_model(args, from_checkpoint)
